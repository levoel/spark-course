...
function VArrow({ label }: { label?: string }) {
  return (
    <div className="flex flex-col items-center py-1">
      {label && (
        <span className="text-[9px] md:text-[10px] text-[var(--ink-muted)] mb-1 text-center leading-tight">
          {label}
        </span>
      )}
      <div className="w-0.5 h-5 bg-[var(--bg-deep)]" />
...
function MergeArrow() {
  return (
    <div className="flex flex-col items-center py-1">
      <div className="w-0.5 h-4 bg-[var(--bg-deep)]" />
      <div className="w-0 h-0 border-l-[5px] border-l-transparent border-r-[5px] border-r-transparent border-t-[7px] border-t-gray-500" />
    </div>
  );
}
      ...
      <div className="flex flex-col items-center gap-3 md:gap-4">
        ...
        <DiagramTooltip content="SQL or DataFrame API -- the entry point for all Spark queries.">
          <div className="bg-blue-500/10 border border-blue-400/30 rounded-xl px-5 py-2.5 backdrop-blur-sm cursor-help text-center">
            <span className="text-sm font-semibold text-blue-700">SQL / DataFrame API</span>
          </div>
        </DiagramTooltip>
        ...
        <DiagramTooltip content="Catalyst generates a logical plan, applies optimization rules, then produces a physical plan with concrete operator implementations.">
          <div className="bg-blue-500/10 border border-blue-400/30 rounded-xl px-5 py-2.5 backdrop-blur-sm cursor-help text-center">
            <span className="text-sm font-semibold text-blue-700">Catalyst Optimizer</span>
            <p className="text-[10px] text-[var(--ink-muted)] mt-0.5">Logical Plan &rarr; Physical Plan</p>
          </div>
        </DiagramTooltip>
          ...
          {isComet ? (
            <DiagramTooltip content="CometScanRule replaces scan operators. CometExecRule traverses the plan bottom-up, replacing supported operators with native Comet equivalents.">
              <div className="bg-cyan-500/10 border border-cyan-400/40 rounded-xl px-5 py-2.5 backdrop-blur-sm cursor-help text-center">
                <span className="text-sm font-semibold text-cyan-700">CometPlugin Intercepts</span>
                <p className="text-[10px] text-[var(--ink-muted)] mt-0.5">CometScanRule + CometExecRule (bottom-up)</p>
              </div>
            </DiagramTooltip>
          ) : (
            <DiagramTooltip content="GlutenPlugin validates the plan at planning time via SparkPlugin API. Supported operators are converted to Substrait plan format.">
              <div className="bg-amber-500/10 border border-amber-400/40 rounded-xl px-5 py-2.5 backdrop-blur-sm cursor-help text-center">
                <span className="text-sm font-semibold text-amber-700">GlutenPlugin Intercepts</span>
                <p className="text-[10px] text-[var(--ink-muted)] mt-0.5">SparkPlugin validates plan at planning time</p>
              </div>
            </DiagramTooltip>
          )}
        </div>
              ...
              <>
                <DiagramTooltip content="Consecutive Comet operators combine into CometNativeExec. The operator tree is serialized to Protocol Buffer format.">
                  <div className="bg-cyan-500/10 border border-cyan-400/30 rounded-lg px-3 py-2 backdrop-blur-sm cursor-help text-center w-full">
                    <span className="text-xs font-semibold text-cyan-700">CometNativeExec</span>
                    <p className="text-[9px] text-[var(--ink-muted)] mt-0.5">ProtoBuf &rarr; JNI</p>
                  </div>
                </DiagramTooltip>
                <MergeArrow />
                <DiagramTooltip content="DataFusion executes the plan natively in Rust with vectorized batch processing and Arrow columnar format.">
                  <div className="bg-cyan-500/10 border border-cyan-400/30 rounded-lg px-3 py-2 backdrop-blur-sm cursor-help text-center w-full">
                    <span className="text-xs font-semibold text-cyan-700">DataFusion (Rust)</span>
                    <p className="text-[9px] text-[var(--ink-muted)] mt-0.5">Vectorized execution</p>
                  </div>
                </DiagramTooltip>
                <MergeArrow />
                <DiagramTooltip content="Results returned to JVM via Arrow FFI (Foreign Function Interface) -- zero-copy transfer of Arrow RecordBatch.">
                  <div className="bg-cyan-500/10 border border-cyan-400/30 rounded-lg px-3 py-2 backdrop-blur-sm cursor-help text-center w-full">
                    <span className="text-xs font-semibold text-cyan-700">Arrow RecordBatch</span>
                    <p className="text-[9px] text-[var(--ink-muted)] mt-0.5">Arrow FFI &rarr; JVM</p>
                  </div>
                </DiagramTooltip>
              </>
            ) : (
              ...
              <>
                <DiagramTooltip content="Gluten converts the physical plan to Substrait format -- a cross-language specification for query plans.">
                  <div className="bg-amber-500/10 border border-amber-400/30 rounded-lg px-3 py-2 backdrop-blur-sm cursor-help text-center w-full">
                    <span className="text-xs font-semibold text-amber-700">Substrait Plan</span>
                    <p className="text-[9px] text-[var(--ink-muted)] mt-0.5">Cross-language plan spec</p>
                  </div>
                </DiagramTooltip>
                <MergeArrow />
                <DiagramTooltip content="Velox (Meta, C++) provides vectorized columnar execution with lazy materialization. ClickHouse backend (Kyligence) offers alternative operator coverage.">
                  <div className="bg-amber-500/10 border border-amber-400/30 rounded-lg px-3 py-2 backdrop-blur-sm cursor-help text-center w-full">
                    <span className="text-xs font-semibold text-amber-700">Velox (C++) / ClickHouse</span>
                    <p className="text-[9px] text-[var(--ink-muted)] mt-0.5">Native execution backend</p>
                  </div>
                </DiagramTooltip>
                <MergeArrow />
                <DiagramTooltip content="Results returned as ArrowColumnarBatch -- Arrow-compatible columnar batches returned to Spark.">
                  <div className="bg-amber-500/10 border border-amber-400/30 rounded-lg px-3 py-2 backdrop-blur-sm cursor-help text-center w-full">
                    <span className="text-xs font-semibold text-amber-700">ArrowColumnarBatch</span>
                    <p className="text-[9px] text-[var(--ink-muted)] mt-0.5">Return to Spark</p>
                  </div>
                </DiagramTooltip>
              </>
            )}
          ...
          <div className="flex flex-col items-center gap-2 flex-1 max-w-[220px]">
            ...
            {isComet ? (
              <DiagramTooltip content="Unsupported operators stay on Tungsten JVM execution. The fallback reason is stored on the Spark plan node for diagnostics (spark.comet.explainFallback.enabled=true).">
                <div className="bg-[var(--bg-surface)] border border-[var(--line-thin)] rounded-lg px-3 py-2 backdrop-blur-sm cursor-help text-center w-full opacity-60">
                  <span className="text-xs font-semibold text-[var(--ink-muted)]">JVM Fallback</span>
                  <p className="text-[9px] text-[var(--ink-subtle)] mt-0.5">Tungsten execution</p>
                  <p className="text-[9px] text-[var(--ink-subtle)]">Reason stored on node</p>
                </div>
              </DiagramTooltip>
            ) : (
              <DiagramTooltip content="Unsupported operators fall back to vanilla Spark JVM execution. ColumnarToRow (C2R) and RowToColumnar (R2C) conversions add overhead at boundaries.">
                <div className="bg-[var(--bg-surface)] border border-[var(--line-thin)] rounded-lg px-3 py-2 backdrop-blur-sm cursor-help text-center w-full opacity-60">
                  <span className="text-xs font-semibold text-[var(--ink-muted)]">JVM Fallback</span>
                  <p className="text-[9px] text-[var(--ink-subtle)] mt-0.5">+ C2R / R2C overhead</p>
                  <p className="text-[9px] text-[var(--ink-subtle)]">ColumnarToRow conversion</p>
                </div>
              </DiagramTooltip>
            )}
          </div>
        ...
        {/* Legend */}
        <div className="flex items-center justify-center gap-4 text-xs text-[var(--ink-muted)] mt-1">
          ...
          <span className="flex items-center gap-1.5">
            <span className="inline-block w-3 h-3 rounded bg-[var(--bg-surface)] border border-[var(--line-thin)] opacity-60" />
            Fallback (JVM)
          </span>
        </div>
      </div>

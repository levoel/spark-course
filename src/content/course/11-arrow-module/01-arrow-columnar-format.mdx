...
<CrossCourseRef
  ...
  lessonSlug="07-arrow/01-memory-layout"
  label="Arrow memory layout: bit-level"
  preview="Как именно Arrow раскладывает данные в памяти — validity bitmaps, offsets buffers, Run-End Encoding. Это фундамент SIMD и zero-copy."
/>

<CrossCourseRef
  courseId="datafusion-course"
  lessonSlug="01-arrow-foundation/01-columnar-format"
  label="DataFusion: Arrow как фундамент"
  preview="Как DataFusion (Rust query engine) использует Arrow в качестве in-memory модели — параллельный взгляд на ту же абстракцию из не-JVM мира."
/>

<CrossCourseRef
  courseId="python-course"
  lessonSlug="09-file-io-formats/04-binary-formats-overview"
  label="Бинарные форматы в Python"
  preview="Обзор Pickle, Protobuf, MessagePack, Arrow IPC, Feather — где Arrow уместен, а где нет."
/>

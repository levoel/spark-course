...
<CrossCourseRef
  ...
  lessonSlug="03-consumers/01-consumer-api"
  label="Kafka Consumer API: основы"
  preview="Как Kafka consumer читает данные, что такое consumer group и offsets — это даёт правильную ментальную модель для Kafka source в Structured Streaming."
/>

<CrossCourseRef
  courseId="kafka-course"
  lessonSlug="03-consumers/06-share-groups-kip932"
  label="Kafka Share Groups (KIP-932)"
  preview="Новая модель потребления в Kafka 4.x — Share Groups, аналог очередей. Альтернатива классическим consumer groups для streaming сценариев."
/>

...
<CrossCourseRef
  ...
  label="Iceberg: catalog и metadata layer"
  preview="Manifest files, manifest list, snapshots, REST catalog (Polaris/Lakekeeper/Unity Catalog OSS) — как устроен metadata-tier Iceberg на уровне файлов."
/>

<CrossCourseRef
  courseId="clickhouse-course"
  lessonSlug="15-integrations/04-iceberg-integration"
  label="ClickHouse + Iceberg: чтение и запись"
  preview="Как ClickHouse читает (и теперь пишет) Iceberg-таблицы через DataLakeCatalog — практический lakehouse-сценарий вне Spark."
/>

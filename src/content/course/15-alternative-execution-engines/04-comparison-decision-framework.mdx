...
└── Нет -> Open-source deployment?
    ├── Нужна простейшая настройка?
    │   └── Да -> Comet (single JAR, Rust safety, 97% test compat)
    ├── Нужна максимальная производительность?
    │   └── Да -> Gluten + Velox (3.3x TPC-H, ByteDance-proven)
    ├── Есть ClickHouse экспертиза?
    │   └── Да -> Gluten + ClickHouse бэкенд
    └── Не уверены -> Начните с Comet (lower risk),
                       оцените Gluten позже
```
